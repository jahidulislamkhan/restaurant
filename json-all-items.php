...
while ( $category = mysqli_fetch_assoc($category_result)){
	...
	if (mysqli_num_rows($sub_category_result) < 1) {
		$items_sql = "SELECT * FROM items where category_id=$category_id";
		$items_result = mysqli_query($db,$items_sql);
		$item_index = 0;
		while ( $item = mysqli_fetch_assoc($items_result)) {
			$item_id = $item['id'];
			$all_items['category'][$category_index]['items'][$item_index] = $item;
			$all_items['category'][$category_index]['items'][$item_index]['recipe'] = array();
			...
			while ( $recipe = mysqli_fetch_assoc($recipe_result)) {
				$recipe_id = $recipe['id'];
				$all_items['category'][$category_index]['items'][$item_index]['recipe'][$recipe_index] = $recipe;
				$recipe_index++;
			}
			$item_index++;
		}
